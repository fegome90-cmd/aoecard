...
- [x] 1.1 Create `Tests/GameCoreTests/PlayLoopTests.swift` scaffold
...
- [x] 1.2 RED — M1-1 resource single-copy (behavioral fail)
...
- [x] 1.3 GREEN — fix `.playResource` site
...
- [x] 1.4 RED — M1-1 unit single-copy (behavioral fail)
  - `testPlayUnitRemovesExactlyOneCopy`: hand = `[unit_a, unit_a]`; perform
    `.playUnit("unit_a")`; assert one copy remains.
- [x] 1.5 GREEN — fix `.playUnit` site (same `firstIndex` + `remove(at:)` pattern).
- [x] 1.6 RED — M1-1 building/tech/special single-copy (behavioral fail, shared case)
  - `testPlayBuildingRemovesExactlyOneCopy`: hand = `[bldg_a, bldg_a]`; perform
    `.playBuilding("bldg_a")`; assert one remains.
- [x] 1.7 GREEN — fix shared `.playBuilding/.playTechnology/.playSpecial` site
    (same pattern).
- [x] 1.8 RED — M1-1 tactic single-copy (behavioral fail)
  - `testPlayTacticRemovesExactlyOneCopy`: tacticsHand = `[tac_a, tac_a]`;
    perform `.playTactic("tac_a")`; assert one remains.
- [x] 1.9 GREEN — fix `.playTactic` site
...
- [x] 1.10a REGRESSION — M1-1 resource not-in-hand rejected, no crash (spec S6)
...
- [x] 1.10b REGRESSION — M1-1 tactic not-in-hand rejected, no crash, no effects (spec S7)
...
- [x] 1.11 REGRESSION — M1-1 payment failure removes nothing (spec scenario 5)
  - `testPaymentFailureRemovesNothing`: hand = `[res_a, res_a]`, cost unpayable;
    perform `.playResource`; assert hand unchanged.
- [x] 1.12 REFACTOR — review the 4 fixed sites. Extract a private helper ONLY
...
- [x] 2.1 RED — M1-4 second resource same turn rejected (BEHAVIORAL fail)
...
- [x] 2.2 GREEN — add field + reset + guard + set
...
- [x] 2.5 REGRESSION — M1-4 failed payment does not consume slot (spec scenario 8)
...
- [x] 2.6 REFACTOR — no semantic change. Confirm the `.playResource` body reads
  in the single mandated order above. Do NOT reorder.

## Phase 3 — Regression + determinism

- [x] 3.1 `swift build --build-tests` exits 0; `swift test` exits 0 with 0
  failures. (Record the test count in the verify report; do NOT pin an absolute
  number in the contract — it goes stale.)
- [x] 3.2 `SimulationTests.testSameSeedProducesIdenticalResult` MUST still pass
  ...
  hardcoded values.
- [x] 3.3 100-game smoke: no `fatalError`/trap. (Full 1.000-game M1-GATE
  harness is a separate change, Phase 6.)

## Phase 4 — Artifact hygiene

- [x] 4.1 `lens_diagnostics` clean on edited files (PlayerState, RulesEngine, PlayLoopTests).
- [x] 4.2 TWO atomic conventional commits inside ONE PR (independent revertability):
...
- [x] All Phase 1 + Phase 2 tasks checked.
- [x] `swift build --build-tests` exits 0; `swift test` exits 0, 0 failures.
- [x] `SimulationTests.testSameSeedProducesIdenticalResult` passes.
- [x] No new public API surface (flag is internal; default value preserves inits).
- [x] Two atomic commits inside one PR; working tree clean (modulo local
  `.vscode/`, `_ctx/`).
